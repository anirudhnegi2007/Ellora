...
export default async function MerchantLayout({
  ...
  const navItems = [
    { name: "Merchant Dashboard", href: "/merchant", icon: LayoutDashboard },
    { name: "Product Inventory", href: "/merchant/products", icon: Package },
      ...
      <aside className="w-64 bg-slate-900/80 border-r border-slate-800 backdrop-blur-xl flex flex-col justify-between p-4 sticky top-0 h-screen z-20">
        <div>
          ...
          <div className="flex items-center justify-between pb-6 mb-6 border-b border-slate-800">
            <Link href="/" className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-purple-500 to-indigo-500 flex items-center justify-center font-bold text-white shadow-lg shadow-purple-500/30">
                E
              </div>
          ...
          <nav className="space-y-1">
            {navItems.map((item) => (
              ...
              >
                <item.icon className="w-4 h-4 text-slate-400 group-hover:text-purple-400 transition-colors" />
                {item.name}
              </Link>
            ))}
            ...
            {user.role === "ADMIN" && (
              <Link
                href="/admin"
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-rose-400 hover:bg-rose-500/10 transition-all font-medium text-sm border border-rose-500/20 mt-4"
              >
        ...
        <div className="border-t border-slate-800 pt-4 space-y-3">
          <div className="flex items-center gap-3 px-2">
            <div className="w-9 h-9 rounded-full bg-purple-600/30 border border-purple-500/40 flex items-center justify-center font-bold text-purple-300">
